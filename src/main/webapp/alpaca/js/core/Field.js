(function($) {
	...
	Alpaca.Field = Base.extend({
		 ...
		 * }
		 */
		constructor: function(container, data, options)
		{
			...
			// backup data
			this.backupData = Alpaca.cloneObject(data);
		...
		lookupTemplate: function()
		{
			...
			if (!template)
			{
				if (this.mode == Alpaca.MODE_VIEW)
				{
					if (this.TEMPLATE_VIEW)
					{
						template = this.TEMPLATE_VIEW;
					}
				}
				
				if (this.mode == Alpaca.MODE_EDIT)
				{
		...
		_render: function()
		{
			...
			// create a new outerEl
			this.outerEl = $("<span></span>");
			$(this.outerEl).addClass("alpaca-field");
			$(this.outerEl).attr({"id": this.getId() + "-field-span"});
			
			// store the field id in an extra dom attribute
		...
		renderOuter: function()
		{
			...
			this.processRender(this.outerEl, function() {
