(function($) {
    ...
    Alpaca.Fields.AddressField = Alpaca.Fields.ObjectField.extend({
		...
		setup: function()
		{
			...
			if (Alpaca.isEmpty(this.settings.addressValidation)) {
				this.settings.addressValidation = true;
			}                        
		},
        ...
        renderField: function(onSuccess) {
			...
			if (this.settings.addressValidation) {
				$('<div class="alpaca-form-button">Map It ! </div>').appendTo(this.getEl()).click(function() {
							...
							geocoder.geocode({
								...
								if (status == google.maps.GeocoderStatus.OK) {
									...
									if ($('#'+mapCanvasId).length == 0) {
										$("<div id='" + mapCanvasId + "' class='alpaca-map-canvas'></div>").appendTo(_this.getEl());
									}
